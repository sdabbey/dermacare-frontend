...
export default function AddStaffModal({ open, onClose }) {
  ...
  const isStepCompleted = (stepIndex) => {
    ...
    if (stepIndex === 2) {
      ...
      return Boolean(Object.values(selectedDays).some(Boolean) && formData.password && formData.confirmPassword);
    }
    return false;
  };
  ...
  formDataToSend.append('working_days', JSON.stringify(Object.keys(selectedDays).filter(day => selectedDays[day])));
    ...
    try {
      const response = await axios.post('https://dermacare-backend.up.railway.app/accounts/staff/create/', 
        formDataToSend,
        {
          headers: {
              ...
              {activeStep === 2 && (
                ...
                <Stack  direction={{sm: "row"}} spacing={2} useFlexGap>
                  <FormControl sx={{flexGrow: '1'}} required>
                    ...
                    <Input sx={{fontSize: '14px'}} placeholder="Default password" type="password" name="password" value={formData.password} onChange={handleChange} />
                  </FormControl>
                  <FormControl sx={{flexGrow: '1'}} required>
                    <FormLabel>Confirm Password</FormLabel>
                    <Input sx={{fontSize: '14px'}} placeholder="Default password" type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} />
                  </FormControl>
                </Stack>
                </>
              )}
