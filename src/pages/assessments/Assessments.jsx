...
export default function Assessment() {
  ...
  React.useEffect(() => {
    const fetchPatientData = async () => {
      ...
      try {
        ...
        if (token){
          const response = await fetch('https://dermacare-backend.up.railway.app/accounts/me/', {
  ...
  React.useEffect(() => {
      ...
      const fetchStaff = async () => {
        try {
          const response = await fetch('https://dermacare-backend.up.railway.app/accounts/doctors/'); // Adjust the API endpoint as needed
          const data = await response.json();
          setStaff(data);
          console.log(data)
        } catch (error) {
